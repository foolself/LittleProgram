...
// float mts = PI/72;//max theta speed.
float mts = 0.04;//max theta speed.
int r = 80;//radius of the circle
int rdtr = 10;//range of the rdt
int rdu = 2;//radius of circle
//**********
	...
	if(mv){
		if(mo){
			for(int i = 0;i<num-1;i++){
				...
				v[i].lerp(mouseX + 2 * (rdt[i]+r) * (sin(mtheta[i]) - 0.5 * sin(2 * mtheta[i])), mouseY-2 * (rdt[i]+r) * (cos(mtheta[i]) - 0.5 * cos(2 * mtheta[i])),0,easing[i]);
				fill(c[i]);
